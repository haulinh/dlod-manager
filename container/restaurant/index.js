...
import apiRestaurant from '../../api-dlod/restaurant';
...
export default function PlaceTravelContainer() {
  ...
  const fetchRestaurant = async () => {
    const result = await apiRestaurant.getRestaurants();
    setData(result);
  };
  ...
  const onFinish = async (values) => {
    await axios.patch(`http://[::1]:3001/restaurants/${idEdit}`, values);
    fetchRestaurant();
  };

  const onCreate = async (values) => {
    await axios.post(`http://[::1]:3001/restaurants`, values);
    fetchRestaurant();
  };
